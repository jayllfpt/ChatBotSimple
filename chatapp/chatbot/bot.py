...
def load_data(n_samples = 1505):
    df1 = pd.read_csv("chatapp/chatbot/Data/S08_question_answer_pairs.txt", sep = '\t')
    df2 = pd.read_csv("chatapp/chatbot/Data/S09_question_answer_pairs.txt", sep = '\t')
    df3 = pd.read_csv('chatapp/chatbot/Data/S10_question_answer_pairs.txt', sep = '\t', encoding = 'ISO-8859-1')
...
def embeddings_data(data: pd.DataFrame, model, load_file = True):
    if load_file:
        data_embeddings = np.load("chatapp/chatbot/embedded-qa.npy")
    else:
